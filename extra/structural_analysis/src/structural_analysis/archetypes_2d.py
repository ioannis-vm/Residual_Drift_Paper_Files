...
from osmg.gen.zerolength_gen import (
    gravity_shear_tab,
    imk_6,
    release_6,
    imk_56,
...
from osmg.ops.element import DispBeamColumn, ElasticBeamColumn, TwoNodeLink
...
def generate_archetype(  # noqa: ANN201, C901, PLR0912, PLR0914, PLR0915
    level_elevs,  # noqa: ANN001
    sections,  # noqa: ANN001
    metadata,  # noqa: ANN001
    archetype,  # noqa: ANN001
    grav_bm_moment_mod,  # noqa: ANN001
    grav_col_moment_mod_interior,  # noqa: ANN001
    grav_col_moment_mod_exterior,  # noqa: ANN001
    lvl_weight,  # noqa: ANN001
    beam_udls,  # noqa: ANN001
    no_diaphragm=False,  # noqa: ANN001, FBT002
    no_llrs=False,  # noqa: ANN001, FBT002
    pinned_beams=False,  # noqa: ANN001, FBT002
):
    """
    ...
    Arguments:
      ...
      pinned_beams:
          If True, beams of SCBFs and BRBFs are pinned at both ends.
    ...
    if lateral_system == 'smrf':
        beam_zerolength_gen = imk_6
    else:  # noqa: PLR5501
        if pinned_beams:
            beam_zerolength_gen = release_6
        else:
            beam_zerolength_gen = imk_6
    ...
    def flatten_dict(dictionary):  # noqa: ANN001, ANN202
        vals = []
    ...
    if lateral_system == 'smrf':
        if no_llrs:
            msg = '`no_LLRS=True` is only available for braced frames.'
    ...
    # system: scbf and brbf
        ...
        for level_counter in range(num_levels):
            ...
            for plcmt_tag_i, plcmt_tag_j in zip(('1', '2'), ('2', '3')):
                ...
                snap_j = 'top_center'
                offset_i = np.array((+0.75 * plate_a[level_counter + 1], 0.00, 0.00))
                offset_j = np.array((-0.75 * plate_a[level_counter + 1], 0.00, 0.00))
                ...
                bcg.add_horizontal_active(
                    ...
                    additional_args={
                        'n_x': n_parameter,
                        'n_y': None,
                        'zerolength_gen_i': beam_zerolength_gen,
                        'zerolength_gen_args_i': {
                            'lboverl': 0.75,
                            'loverh': 0.50,
                            'rbs_factor': None,
                        ...
                        'zerolength_gen_j': beam_zerolength_gen,
                        'zerolength_gen_args_j': {
                            'lboverl': 0.75,
                            'loverh': 0.50,
                            'rbs_factor': None,
            ...
            for plcmt_i, plcmt_j in zip(('2', '2'), ('1', '3')):
                ...
                if lateral_system == 'scbf':
                    ...
                    brace_mat = mtlg.generate_steel_hss_circ_brace_fatigue_mat(
                        brace_sec,
                        brace_phys_mat,
                        metadata['brace_buckling_length'][level_counter + 1],
                    )
                    ...
                    if not no_llrs:
                        bcg.add_diagonal_active(
                            ...
                            'generate_hinged_component_assembly',
                            {
                                ...
                                'zerolength_gen_args_i': {
                                    'distance': metadata['hinge_dist'][
                                        level_counter + 1
                                    ],
                                    'element_type': TwoNodeLink,
                                    'physical_mat': steel_phys_mat,
                                    'd_brace': bsec.properties['OD'],
                                    'l_c': metadata['brace_l_c'][level_counter + 1],
                                    't_p': metadata['gusset_t_p'][level_counter + 1],
                                    'l_b': metadata['gusset_avg_buckl_len'][
                                        level_counter + 1
                                    ],
                                },
    ...
    for level_counter in range(num_levels):
        ...
        top_node = next(iter(col_assembly.external_nodes.items()))[1]
    ...
    for i in range(num_levels + 1):
        p_nodes.append(query.search_node_lvl(x_grd['LC'], 0.00, i))  # noqa: PERF401
    ...
    if lateral_system == 'smrf':
        ...
        msg = f'Invalid system: {lateral_system}'
        raise ValueError(msg)
...
def smrf_3_ii(direction, no_llrs=False) -> tuple[Model, LoadCase]:  # noqa: ANN001, FBT002
    ...
    doubler_plate_thicknesses = {
        'outer_frame': {
            'exterior': {'level_1': 0.8750, 'level_2': 0.3125, 'level_3': 0.2500},
            'interior': {'level_1': 1.8125, 'level_2': 0.7500, 'level_3': 0.6250},
        }
    }
    ...
    beam_udls = {'level_1': 74.0, 'level_2': 74.0, 'level_3': 74.0}  # lb/in
...
def smrf_3_iv(direction, no_llrs=False) -> tuple[Model, LoadCase]:  # noqa: ANN001, FBT002
    ...
    doubler_plate_thicknesses = {
        'outer_frame': {
            'exterior': {'level_1': 1.00, 'level_2': 1.00, 'level_3': 0.25},
            'interior': {'level_1': 1.75, 'level_2': 1.75, 'level_3': 0.00},
        },
        'inner_frame': {
            'exterior': {'level_1': 0.5000, 'level_2': 0.5000, 'level_3': 0.5000},
            'interior': {'level_1': 0.9375, 'level_2': 0.9375, 'level_3': 1.0000},
        },
    }
...
def smrf_6_ii(direction, no_llrs=False) -> tuple[Model, LoadCase]:  # noqa: ANN001, FBT002
...
def smrf_6_iv(direction, no_llrs=False) -> tuple[Model, LoadCase]:  # noqa: ANN001, FBT002
...
def smrf_9_ii(direction, no_llrs=False) -> tuple[Model, LoadCase]:  # noqa: ANN001, FBT002
...
def smrf_9_iv(direction, no_llrs=False) -> tuple[Model, LoadCase]:  # noqa: ANN001, FBT002
...
def scbf_3_ii(
    ...
    no_llrs=False,  # noqa: ANN001, FBT002
    pinned_beams=False,  # noqa: ANN001, FBT002
) -> tuple[Model, LoadCase]:  # noqa: ANN001, FBT002, RUF100
    """
...
def brbf_3_ii(
    ...
    sections = {
        ...
        'braces': {'level_1': '7.00', 'level_2': '5.50', 'level_3': '4.00'},
    }
...
def brbf_3_iv(
    ...
    sections = {
        ...
        'braces': {'level_1': '5.75', 'level_2': '4.25', 'level_3': '3.00'},
    }
...
def brbf_1_ii(
    ...
    """
    1 story special buckling restrained braced frame risk category II
    archetype.
    """  # noqa: D205, RUF100
    if direction == 'x':
        grav_bm_moment_mod = 5.50
    ...
    level_elevs = np.array((15.00,)) * 12.00
