...
class Model:
    ...
    def fit(
        self,
        method: Literal['mle', 'quantiles', 'bayesian', 'mle-fast'] = 'mle',
        *,
...
class Model_Bilinear_LogNormal(BilinearModel):
    """
    LogNormal model
    """
    ...
    def __init__(self) -> None:
        super().__init__()
        # initial parameters
        self.parameters = np.array((0.008, 0.30, 0.20))
        # parameter names
        self.parameter_names = ['pid_0', 'delta_slope', 'sigma']
        # bounds
        self.parameter_bounds = [(0.00, 0.02), (0.00, 1.00), (0.01, 1.00)]
    ...
    def evaluate_pdf(
        ...
        assert self.parameters is not None
        _, _, sigma = self.parameters
        bilinear_fnc_val = self.bilinear_fnc(pid)
        log_rid = np.log(rid)
        pdf_val = sp.stats.norm.pdf(log_rid, np.log(bilinear_fnc_val), sigma)
    ...
    def evaluate_cdf(self, rid: npt.NDArray, pid: npt.NDArray) -> npt.NDArray:
        ...
        bilinear_fnc_val = self.bilinear_fnc(pid)
        log_rid = np.log(rid)
        return sp.stats.norm.cdf(log_rid, np.log(bilinear_fnc_val), sigma)

    def evaluate_inverse_cdf(self, quantile: float, pid: npt.NDArray) -> npt.NDArray:
        assert self.parameters is not None
        _, _, sigma = self.parameters
        bilinear_fnc_val = self.bilinear_fnc(pid)
        return np.exp(sp.stats.norm.ppf(quantile, np.log(bilinear_fnc_val), sigma))
...
class Model_Trilinear_LogNormal(Model):
    """
    LogNormal model
    """
    ...
    def __init__(self) -> None:
        super().__init__()
        # initial parameters
        self.parameters = np.array((0.005, 0.15, 0.020, 0.40, 0.20, 0.30))
        # parameter names
    ...
    def obtain_delta_and_sigma(
        ...
        # calculate m0 for sigma
        sigma_slope_0 = (sigma_1 - sigma_0) / (pid_1 - pid_0)
        delta_trilinear: npt.NDArray = self.trilinear_fnc(
            pid, 1e-6, 0.00, delta_slope_0, delta_slope_1, pid_0, pid_1
        )
        sigma_trilinear: npt.NDArray = self.trilinear_fnc(
            pid, sigma_0, 0.00, sigma_slope_0, 0.00, pid_0, pid_1
        )
        return delta_trilinear, sigma_trilinear
    ...
    def evaluate_pdf(
        ...
        delta_trilinear, sigma_trilinear = self.obtain_delta_and_sigma(pid)
        log_rid = np.log(rid)
        pdf_val = sp.stats.norm.pdf(
            log_rid, np.log(delta_trilinear), sigma_trilinear
        )
        pdf_val[pdf_val < 1e-6] = 1e-6  # noqa: PLR2004
        if censoring_limit:
            censored_range_mass = self.evaluate_cdf(
    ...
    def evaluate_cdf(self, rid: npt.NDArray, pid: npt.NDArray) -> npt.NDArray:
        delta_trilinear, sigma_trilinear = self.obtain_delta_and_sigma(pid)
        log_rid = np.log(rid)
        return sp.stats.norm.cdf(log_rid, np.log(delta_trilinear), sigma_trilinear)

    def evaluate_inverse_cdf(self, quantile: float, pid: npt.NDArray) -> npt.NDArray:
        delta_trilinear, sigma_trilinear = self.obtain_delta_and_sigma(pid)
        return np.exp(
            sp.stats.norm.ppf(quantile, np.log(delta_trilinear), sigma_trilinear)
        )
...
class Model_Trilinear_Weibull(Model):
    ...
    def get_mle_objective_fast(
        ...
        if np.any(mask_censored):
            ...
            small_mask = s_c < 1e-6  # noqa: PLR2004
    ...
    def get_mle_gradient_numeric(self, parameters: np.ndarray) -> np.ndarray:
        self.parameters = parameters
        baseline = self.get_mle_objective(parameters)
        step = 1e-8
        grad = np.empty(len(parameters))
        for i in range(len(parameters)):
            parameters[i] += step
            self.parameters = parameters
            grad[i] = self.get_mle_objective(parameters)
            parameters[i] -= step
            self.parameters = parameters
        grad -= baseline
        grad /= step
...
class ResidualDriftTrilinearWeibullModel:
    ...
    def _fit_single_model(
        ...
        global_search: bool = True,  # noqa: FBT001, FBT002
    ...
    def _estimate_model_corr(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.fit_directions_together:
            ...
            if rid_matrix.shape[0] < 2:  # noqa: PLR2004
                return pd.DataFrame(
                    np.eye(len(self.locations)),
                    index=self.locations,
                    columns=self.locations,
        ...
        for direction in rid_matrix.columns.get_level_values('dir').unique():
            ...
            if rid_dir.shape[0] < 2:  # noqa: PLR2004
                continue
    ...
    def fit(self, psdr_rsdr_data: pd.DataFrame) -> None:  # noqa: C901
        """
        ...
        if self.approach == 'max_max':
            group_levels = [name for name in data.index.names if name not in {'loc'}]  # noqa: FURB171
            if not group_levels:
                msg = (
...
class Model_P58(Model):
    ...
    def delta_fnc(self, pid: npt.NDArray, delta_y: float) -> npt.NDArray:  # noqa: PLR6301
    ...
    def set(self, delta_y=0.00, beta=0.00) -> None:  # noqa: ANN001
        """
    ...
    def evaluate_inverse_cdf(self, quantile: float, pid: npt.NDArray) -> npt.NDArray:
        ...
        -------
        npt.NDArray
          Inverse of the conditional RID|PID CDF.
        """
        assert self.parameters is not None
